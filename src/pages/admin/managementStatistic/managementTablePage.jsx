...
const ManagementTable = () => {
  ...
  useEffect(() => {
    const fetchPapers = async () => {
      try {
        ...
        console.log("Processed papers:", approvedPapers); // Debugging log
      } catch (error) {
    ...
    const getUserInfo = () => {
      ...
      setUserRole(storedUser.role || "admin");
  ...
  const filteredPapers = papers.filter((paper) => {
    ...
    // Debugging logs for each condition
    console.log("Checking paper:", paper);
    console.log(
      "Condition 1 (Paper Type):",
      filterPaperType === "Tất cả" || paper.articleType === filterPaperType
    );
    console.log(
      "Condition 2 (Group):",
      filterGroup.includes("Tất cả") || filterGroup.includes(paper.groupName)
    );
    console.log(
      "Condition 3 (Title):",
  ...
  console.log("Filtered papers after applying conditions:", filteredPapers);
