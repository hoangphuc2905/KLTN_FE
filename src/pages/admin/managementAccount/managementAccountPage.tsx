...
const UserManagement = () => {
  ...
  const uniquePositions = ["Tất cả", ...new Set(displayedUsers.map(user => user.position))];
  const uniqueDepartments = ["Tất cả", ...new Set(displayedUsers.map(user => user.department))];
  const uniqueStatuses = ["Tất cả", ...new Set(displayedUsers.map(user => user.status))];
      ...
      <div className="flex flex-col pb-7 pt-[80px] max-w-[calc(100%-220px)] mx-auto">
        ...
        <div className="self-center w-full max-w-[1563px] px-6 mt-4">
          <div className="flex items-center gap-2 text-gray-600">
            <img src="https://cdn-icons-png.flaticon.com/512/25/25694.png" alt="Home Icon" className="w-5 h-5" />
            <span>Trang chủ</span>
            <span className="text-gray-400"> &gt; </span>
            <span className="font-semibold text-sky-900">Quản lý người dùng</span>
          </div>
        </div>
          ...
          <div className="flex border-b">
            <button
              className={`px-8 py-3 text-center ${
                activeTab === "user" ? "bg-[#00A3FF] text-white" : "bg-white text-gray-700"
              } rounded-t-lg`}
            ...
            <button
              className={`px-8 py-3 text-center ${
                activeTab === "admin" ? "bg-[#00A3FF] text-white" : "bg-white text-gray-700"
              } rounded-t-lg`}
...
{showFilter && (
  <div className="absolute top-full mt-2 z-50 shadow-lg">
    <form className="relative px-4 py-5 w-full bg-white max-w-[500px] max-md:px-3 max-md:py-4 max-sm:px-2 max-sm:py-3">
      ...
      <div className="mb-3">
        <label className="block text-gray-700">MSSV/MSGV:</label>
        <input
          type="text"
          value={filterId}
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-[#F5F7FB] text-left">
                      <th className="px-6 py-4 text-gray-700 font-medium">STT</th>
                      <th className="px-6 py-4 text-gray-700 font-medium">HỌ VÀ TÊN</th>
                      <th className="px-6 py-4 text-gray-700 font-medium">CHỨC VỤ</th>
                      <th className="px-6 py-4 text-gray-700 font-medium">KHOA</th>
                      <th className="px-6 py-4 text-gray-700 font-medium">MSSV/MSGV</th>
                      <th className="px-6 py-4 text-gray-700 font-medium">QUYỀN</th>
                      <th className="px-6 py-4 text-gray-700 font-medium">TRẠNG THÁI</th>
                      <th className="px-6 py-4 text-gray-700 font-medium">CHỈNH SỬA</th>
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2 py-1 rounded ${
                              user.status === "Hoạt động" ? "text-green-700" : "text-red-700"
                            }`}
                          >
                            {user.status}
                          </span>
